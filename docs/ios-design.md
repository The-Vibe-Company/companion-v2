...
### Navigation contract
The roster is the entry point for every Companion: a single row tap opens its chat. Long press exposes Settings alongside Duplicate, Move to, and Delete; Settings opens the one details page. The chat header remains a centered CharacterMark-and-name pill that opens that page too. Details presents character, name, instructions, provider/model, routines and run history, notifications, Skills, plugins and selected MCP accounts, triggers, runtime controls, and Owner-only deletion as peer cards in one scroll. Triggers are results, not setup instructions: Companion reuses the matching attached MCP credential, registers the provider webhook remotely, and shows provider, notify/relay mode, registered/failed status, retry, and fire history. A platform webhook URL is a collapsed technical detail for editors only; no primary iOS flow asks a user to paste a URL or separate provider key. Its explicit Open chat action replaces the detail route, preserving a single chat/details cycle. Member settings remains reachable only from the account avatar; there are no separate connected-resources or legacy Companion-settings destinations. The root NavigationStack installs the native interactive pop plus a supplemental leading-edge capture once, so pushed chat, details, computer, and history surfaces share guarded back behavior without intercepting horizontal content scrolling.
