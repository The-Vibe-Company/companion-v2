...
- Row: mark 36pt — title 17 semibold — preview one line 15 regular secondary — trailing timestamp (hierarchical: 8:41 AM today, Yesterday, weekday, 8/19) — 8pt status dot in its own fixed slot with small trailing padding before the chevron — blue unread dot 6pt right-aligned when unread. The status dot is vertically centered on the title line, so timestamp width never shifts it relative to the trailing edge. The status mapping is green for running, pulsing green for Pi-acknowledged replying, red for runtime error, and gray for every stopped/starting/stopping/not-created projection; archived Boxes are normalized to stopped by the API. No hairline between rows.
...
### Navigation contract
The roster is the entry point for every Companion: a single row tap opens its chat. Long press exposes Settings alongside Duplicate, Move to, and Delete; Settings opens the one details page. The chat header remains a centered CharacterMark-and-name pill that opens that page too. Details presents character, name, instructions, provider/model, routines and run history, notifications, Skills, plugins and selected MCP accounts, triggers, runtime controls, and Owner-only deletion as peer cards in one scroll. Triggers are results, not setup instructions: Companion reuses a member-wide trigger provider account, registers the provider webhook remotely, and shows provider, notify/relay mode, registered/failed status, retry, and fire history. GitHub and Sentry provider accounts normally share the encrypted OAuth credential already held by their MCP account; Linear and supported fallbacks accept one encrypted API key at member level. Connecting once makes the provider live for every Companion with no per-Companion attachment step and no duplicate credential prompt. A platform webhook URL is a collapsed technical detail for editors only; no primary iOS flow asks a user to paste a URL or configure a provider console. Its explicit Open chat action replaces the detail route, preserving a single chat/details cycle. Member settings remains reachable only from the account avatar; there are no separate connected-resources or legacy Companion-settings destinations. The root NavigationStack installs the native interactive pop plus a supplemental leading-edge capture once, so pushed chat, details, computer, and history surfaces share guarded back behavior without intercepting horizontal content scrolling.
