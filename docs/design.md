...
A Companion carries optional instructions of at most 280 characters, also shown as its short persona in the list. Creation asks for the name, those instructions, and one compact picker over connected entries from the shared Pi provider catalog, defaulting to the workspace default. The same connected-provider picker is reused by `/companions/:id/settings`, so adding Kimi or another catalog connection updates both surfaces without a second picker. Owner and Editor can later change all three on that separate web and mobile-web page entered from the list; Viewer cannot open it, and only Owner can delete. Saving settings never restarts or wakes a Box; an online provider change rewrites auth and recycles only Pi. Updated instructions are staged as Pi's appended system prompt on the next start. Delete first claims `stopping` to block further wakes, then stops and archives the deterministically named `Companion <uuid>` Box before removing its metadata. Native mobile clients get the Companion list and its thread only; Skills, Plugins, and Companion settings stay on web and mobile web. Pi/Box controls, provider catalogs, and desktop chrome remain invisible.
...
systemd start. A provider-auth replacement restarts Pi after rewriting `auth.json`; other cold paths
keep idempotent `systemctl start` so they do not interrupt an active turn. Desktop URLs are
