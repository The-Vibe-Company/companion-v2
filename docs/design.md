...
A Companion carries optional instructions of at most 280 characters, also shown as its short persona in the list. Creation asks for the name, those instructions, then one connected provider and one model from that provider's pinned Pi catalog. The same provider-then-model picker is reused by `/companions/:id/settings`, and the catalog response supplies both lists. Owner and Editor can later change all four settings on that separate web and mobile-web page entered from the list; Viewer reads the selected model but has no writes, and only Owner can delete. Saving settings never wakes or restarts a Box: an online provider or model change recycles only Pi, while an asleep Companion applies it on the next start. Provider changes also select the new provider's default model and rewrite auth; model-only changes preserve the credential generation. Updated instructions are staged as Pi's appended system prompt on the next start. Delete first claims `stopping` to block further wakes, then stops and archives the deterministically named `Companion <uuid>` Box before removing its metadata. Native mobile clients get the Companion list and its thread only; Skills, Plugins, and Companion settings stay on web and mobile web. Pi/Box controls, provider catalogs, and desktop chrome remain invisible.
