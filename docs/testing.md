...
| Skill runs, saved configurations, inputs, prompts, events, and prompt-linked attachments stay creator-only | An admin or another member reads or mutates a private run, or a file is detached from the message that introduced it | `runsDb.integration.test.ts` | Postgres non-superuser | Relaxing a creator-through-parent policy or the prompt attachment foreign key makes the same-tenant admin or attachment-association assertions fail |
