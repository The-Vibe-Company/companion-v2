...
| One Project resumes one persistent sandbox and safely multiplexes independent OpenCode Sessions | A worker creates one sandbox per Session, duplicates a prompt after a crash, mixes event streams, attributes a pending VM to the prior billing revision, or loses Files after idle suspension | Project supervisor/runtime tests plus the credential-gated Vercel smoke | Core + worker + provider contract | Changing the deterministic name, pending-admission token, ambiguous-activation observation, message reconciliation, single subscription, or checkpoint restore fails identity, billing, concurrency, and resume assertions |
