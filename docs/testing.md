...
| The one-shot cutover settles storage before rows and never discards provider-backed rows unasked | Objects stranded with no reference, or an unconfirmed run deleting them | Historical PostgreSQL migration replay + object deletion order | Empty the retired tables before deleting the objects they name |
