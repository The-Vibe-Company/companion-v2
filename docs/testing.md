...
| Interactive HTML artifacts load their relative site without gaining Companion authority | A linked script/image fails after freeze, a ticket crosses runs or tenants, copied capabilities cause unbounded load, a TLS proxy bypasses preview-host routing, browser clock drift breaks expiry, or untrusted HTML reaches cookies/control-plane mutations | `runHtmlPreview.test.ts`, `runArtifacts.test.ts`, `runRoutes.test.ts`, `RunChatView.test.ts`, `runsDb.integration.test.ts` | Core + storage + HTTP + browser component + Postgres | Removing UTF-8 classification, ticket expiry/signing and clock-skew bound, per-ticket request/byte/concurrency budgets, monotonic client expiry, exact-path creator revalidation, hostname routing, opaque-origin mutation rejection, cookie bypass, or iframe/CSP sandbox makes the corresponding fixture or isolation assertion fail |
