...
| Managed billing reconciliation recovers without a redeploy | A transient Stripe or database failure at worker boot permanently disables seat synchronization, or a scheduled rejection becomes unhandled | `billingSupervisor.test.ts` | Worker unit | Removing startup rescheduling, periodic error isolation, or stop-time timer cleanup fails the recovery scenarios |
