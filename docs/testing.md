...
| Follow-ups remain FIFO across reloads and stopping one turn cannot stop its successor or destroy the retained session | A queued message disappears, a late stop aborts the next prompt, or partial output is lost with the sandbox | `runsDb.integration.test.ts`, `runSupervisor.test.ts`, `runRoutes.test.ts`, `RunChatView.test.ts` | HTTP + Postgres + worker + browser component | Removing the queue bound/exact prompt identity, cancellation CAS, stop-protocol gate, partial snapshot barrier, or pending-prompt render makes the concurrency and reload scenarios fail |
