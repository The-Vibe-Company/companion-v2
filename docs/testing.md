...
| Skill runs, saved configurations, inputs, prompts, events, and attachments stay creator-only | An admin or another member reads or mutates a private run | `runsDb.integration.test.ts` | Postgres non-superuser | Relaxing a creator-through-parent policy makes the same-tenant admin visibility assertions fail |
