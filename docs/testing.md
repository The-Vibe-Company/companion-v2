...
| Skill runs, saved configurations, inputs, prompts, events, attachments, and artifacts stay creator-only | An admin or another member reads or mutates a private run | `runsDb.integration.test.ts` | Postgres non-superuser | Relaxing a creator-through-parent policy makes the same-tenant admin visibility assertions fail |
| The durable run queue has one lease owner and resumes without duplicating commands | Two workers start the same sandbox or prompt after a crash | `runsDb.integration.test.ts` | Core + Postgres | Weakening claim, heartbeat, reclaim, or cleanup ownership makes the lease transition assertions fail |
