...
/**
 * Envelope encryption for write-only secrets (provider connection API keys and the per-run
 ...
 *
 * Both blobs serialize as `v1:<b64 iv>:<b64 tag>:<b64 data>`. The AAD (`orgId:ownerId:key`) binds a
...
export function parseSecretsKey(raw: string | undefined): Buffer {
  if (!raw || raw.trim() === "") {
    throw new SecretBoxError("COMPANION_SECRETS_KEY is not set (base64 32-byte key required for stored secrets)");
  }
...
/** AAD binding a secret to its owning row: `${orgId}:${ownerId}:${key}` (e.g. run server password). */
export function secretAad(orgId: string, ownerId: string, key: string): string {
