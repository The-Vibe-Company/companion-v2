 ...
 * (envelope-encrypted, write-only), which enables that provider's models in the run launcher's picker
 * and is resolved live at serve time (personal beats workspace) — never copied onto a run. Same load-order reasoning as
...
/**
 * Decrypt the key that should reach a run for a provider: the LAUNCHER's personal connection wins,
