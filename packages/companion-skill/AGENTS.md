...
- Validate with `node packages/companion-skill/scripts/check-version-bump.mjs` and `python3 -m unittest discover -s packages/companion-skill/skill/scripts -p 'test_*.py'`. Run the relevant TypeScript client tests for changes under `packages/companion-skill/client/`.
