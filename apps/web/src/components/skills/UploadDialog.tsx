...
export function InstallDialog({ skill, onClose }: { skill: SkillVM; onClose: () => void }) {
  ...
  // The Companion installer reads packages and performs the non-replayable secret retrieval flow.
  const ensureToken = useCallback(async () => {
    if (token) return token;
...
Use the installed Companion helper skill to install this exact skill and version into the chosen local skills folder. Its workflow must run the server secret preflight, stop if required configuration is missing, ask for one global confirmation, redeem the one-time grant only after confirmation, and commit the package with its .env projection atomically. Do not print, log, or persist any secret value outside that projection. Report warnings for optional secrets.
