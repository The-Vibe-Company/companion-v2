...
export default async function SkillsPage() {
  ...
  const { data: mem } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  const workspace = ((mem?.organizations ?? null) as { slug?: string } | null)?.slug ?? "workspace";

  // Only the current user's teams (org admins can see everyone's rows via RLS, so filter).
  ...
  return <SkillsApp initialSkills={skills} me={me} teams={teams} workspace={workspace} />;
}
