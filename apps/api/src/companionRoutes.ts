...
import {
  ...
  settleExpiredCompanionDecisions,
...
import {
  ...
  deliverCompanionMessages as deliverCompanionMessagesViaRuntime,
  isBoxRuntimeFailure,
  recordProjection,
  startCompanionRuntime,
  stopCompanionRuntime,
  type CompanionBoxRuntime,
  type CompanionLifecycleContext,
...
export function registerCompanionRoutes(
  ...
  async function lifecycleContext(
    ...
    const orgId = await orgIdFromContext(c);
    return { actor, orgId, env, runtimeFactory };
  }
  ...
  app.get("/v1/companions/:id/thread", async (c) => {
    try {
      ...
      // Settle overdue chips and expired permission cards here without contacting or waking the
      // Box. The FIFO responses for expired cards are dropped deliberately: this route never
      // touches Box, and the next live sync (or the worker sweep) hands them to Pi.
      const thread = await tenant(c, async ({ actor, orgId, database }) => {
        await expireCompanionToolRuns({ actor, orgId, companionId, database });
        await settleExpiredCompanionDecisions({ actor, orgId, companionId, database });
  ...
  app.post("/v1/companions/:id/messages", async (c) => {
    try {
      ...
      const sent = await tenant(c, async ({ actor, orgId, database }) => {
        ...
        // A permission card past its deadline is a Deny before this turn starts. Its FIFO
        // responses ride along so a runtime this send reaches can unblock Pi before the prompt.
        // Settlement only: this path reads its own thread and must not build one to discard.
      ...
      // An ask_user card that expired while the Box slept still blocks Pi on its FIFO answer.
      // Hand the cancellations over before the prompt so the stale question cannot stall the
      // turn this send is starting. Best-effort: a failure here is repaired by the next sync.
      for (const response of sent.decisionResponses) {
