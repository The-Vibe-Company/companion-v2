...
describe("companion skill package + row", () => {
  ...
  it("builds a 'none' row with an actionable install prompt for a fresh caller", async () => {
    ...
    expect(changelog).toContain("Companion replies gained the reasoning that produced them");
    expect(changelog).toContain("Companion transcripts and reply reasoning outside delegated");
