...
describe("companion skill package + row", () => {
  ...
  it("builds a 'none' row with an actionable install prompt for a fresh caller", async () => {
    ...
    expect(changelog).toContain("member-scoped");
    expect(changelog).toContain("same encrypted GitHub or Sentry MCP OAuth credential");
    expect(changelog).toContain("unregistered triggers");
