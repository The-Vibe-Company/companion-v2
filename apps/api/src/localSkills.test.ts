...
describe("companion skill package + row", () => {
  ...
  it("builds a 'none' row with an actionable install prompt for a fresh caller", async () => {
    ...
    expect(changelog).toContain("shared run-artifact contracts");
    expect(changelog).toContain("without changing");
    expect(changelog).toContain("supported API operations");
