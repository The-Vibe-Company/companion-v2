...
describe("companion skill package + row", () => {
  ...
  it("builds a 'none' row with an actionable install prompt for a fresh caller", async () => {
    ...
    expect(changelog).toContain("a projected last chat line for the conversation list");
    expect(changelog).toContain("no tool run or pending permission question reaches a list preview");
