...
export function deterministicRunAttachmentId(input: {
  orgId: string;
  actorId: string;
  ...
  const digest = createHash("sha256")
    .update("companion-run-attachment:v1\0")
    .update(input.orgId)
    .update("\0")
...
/** Conditional creation makes retry/conflict cleanup incapable of deleting an earlier run's file. */
export async function putRunAttachmentOnce(input: {
  key: string;
  body: Buffer;
  contentType: string;
