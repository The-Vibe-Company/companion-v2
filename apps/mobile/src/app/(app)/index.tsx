...
export default function CompanionListScreen() {
  ...
  return (
    <View className="flex-1 bg-background pt-safe">
      ...
      {gateUnavailable ? (
        ...
        <FlatList
          ...
          contentContainerStyle={[
            styles.listContent,
            (poll.data?.length ?? 0) === 0 ? styles.emptyListContent : undefined,
          ]}
          ListEmptyComponent={(
            <EmptyState
      ...
      <View pointerEvents="box-none" className="absolute bottom-0 right-0 p-4 pb-safe-offset-4">
        <Pressable
          onPress={() => router.push("/(app)/profile")}
          accessibilityRole="button"
          accessibilityLabel="Profile"
          className="min-h-12 flex-row items-center gap-2 rounded-full border border-border bg-surface px-4 active:bg-surface-raised"
        >
          <Ionicons name="person-circle-outline" size={22} className="text-foreground" />
          <Text className="text-sm font-semibold text-foreground">Profile</Text>
        </Pressable>
      </View>
    </View>
  );
