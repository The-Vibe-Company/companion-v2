...
- **For greenfield work without local assets, create or source imagery through host-authorized tools.** Prefer generated raster assets or a project-approved stock library, verify usage rights and visual fit, then store the chosen asset locally with the project instead of hotlinking a third-party service. If no authorized imagery path exists, state the constraint rather than inventing remote URLs or substituting colored `<div>` placeholders. <!-- rule:brand-imagery-local-assets -->
