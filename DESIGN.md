...
The sidebar presents the Skills libraries directly: My Skills, Organization, Installed, Companion skills, Archived, and Secrets. When `COMPANION_COMPANIONS_ENABLED=true`, it also presents a Companions entry for the empty-list shell; the entry is absent by default. The Box/Pi harness is API-only and must not render as agent launcher, session history, provider catalog, desktop control, or runtime chrome in this shell. External coding-agent access lives in Settings and is described explicitly as delegated Skills Hub access.
