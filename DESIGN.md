...
thread gets one accent `New` divider on the first message somebody else wrote past their own read
watermark, which is the member-private one the list badge uses rather than a per-device guess;
