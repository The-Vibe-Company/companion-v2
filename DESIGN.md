...
**Companion thread** is one transcript in one reading column narrower than the page, with two sides.
A writer keeps the floor across consecutive turns, so the writer and the time appear once per passage
and the turns under it are only the words. A member's message is an accent-tinted bubble on the right
and a Companion reply is a hairline-bordered `surface` bubble on the left; each is at most about
three quarters of the column, and the corner nearest its writer stays square so a passage reads as one
side of a conversation rather than a stack of identical cards. A run note is one quiet muted line, and
tool chips and permission cards stay full-width chrome between turns rather than becoming a third
bubble. The transcript is parted by day, once per day it was written in, and a reader returning to a
thread gets one accent `New` divider on the first message somebody else wrote since they last read it;
neither separator ever lands on a tool chip, which is chrome inside a turn. Loading uses static
skeleton lines, and a running Box that owes a reply says so as a muted line, never as an animated
indicator. The composer is one pill-shaped field with a round accent send control inside its right
edge, one hint line underneath, and no toolbar: no attachments, dictation, slash commands, mentions,
model picker, or tool controls belong in the thread. A Viewer gets the same transcript with a
read-only note in place of the composer. Empty threads state what happens next instead of greeting the
reader, and there is no centered welcome panel and no thread list inside the thread; the workspace
sidebar is where conversations are listed.
