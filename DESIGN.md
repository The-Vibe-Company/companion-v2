...
The sidebar presents the Skills libraries directly: My Skills, Organization, Installed, Companion skills, Archived, and Secrets. When `COMPANION_COMPANIONS_ENABLED=true`, it also presents a Companions entry for the list/create shell; the entry is absent by default. This shell may show connected-provider metadata, one compact credential form for Owner/Admin, a short provider picker at creation, a focused share modal for Companion Owners, and a control-plane transcript drawer. Viewer drawers are explicitly read-only and never expose run, plugins, or desktop controls. It must not render the Box/Pi harness, a full provider catalog, or raw runtime chrome. External coding-agent access lives in Settings and is described explicitly as delegated Skills Hub access.
